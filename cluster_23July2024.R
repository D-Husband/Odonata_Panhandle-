...
biplot(pca.1$scores, pca.1$loadings, cex=0.5, main = "PCA", xlab="Component 1", ylab="Component 2")
# Scree plot:
...
d <- read_excel("sampling_data.xlsx", sheet = "full_data")
# The data in d represents number of species first observed in 5-min intervals
# up to 60 min from start. Four-letter species abbreviations are as in Figure 5.
# DDLat and DDLong are decimal degree coordinates of each location. Sites C1 & C2
# were at Clapp Park (Site Code U5), G1 & G2 at Guy Park (U9), A1 & A2 at 
# Abernathy Community Park (U1), and M1 & M2 were at Muleshoe City Park (U21).

d <-
  d %>%
  select(-(c(DDLat:DDLong, SiteCode)))

# Convert TIME1stAPPEAR to a factor and put in correct order:
d$Site <- factor(d$Site)
d$Location <- factor(d$Location)
d$TIME1stAPPEAR <- factor(d$TIME1stAPPEAR)

# Reorder factor levels of TIMEstAPPEAR:
d$TIME1stAPPEAR <- factor(d$TIME1stAPPEAR, levels = c("0-5","5-10","10-15","15-20","20-25","25-30", "30-35", "35-40", "40-45","45-50", "50-55", "55-60"))

# Rename Muleshoe_Lake in d to Muleshoe:
levels(d$Site)[4] <- "Muleshoe"

# Melt data into long format for analysis:
d.long <- 
  d %>%
  pivot_longer(cols = ANJU:ISRA, 
               names_to = "species",
               values_to = "observation")

# Calculate species numbers at different time intervals at each site at each wetland:
totals <- 
  d.long %>% 
  group_by(Site, Location, TIME1stAPPEAR) %>%
  summarise(obsTotal = sum(observation)) %>%
  mutate(sumSpecies = cumsum(obsTotal))

View(totals)

ggplot(totals, aes(TIME1stAPPEAR, sumSpecies, group = Location, col = Location)) +
  geom_line() +
  facet_wrap(~Site) +
  labs(x = "Time interval (minutes)", y = "Number of species") +
  theme_bw() +
  theme(panel.grid = element_blank(), strip.background = element_blank(), axis.text.x = element_text(angle = 270))
...
par(mfrow = c(1, 3))
ordiplot(RDA.hp, display=c('si', 'cn'), type = 'n', main="Hydroperiod")
points(RDA.hp, display='si', col = c("red", "black"), pch=20, cex=2)
legend('bottomright', col=c("red", "black"), pch=20, cex=1.5, legend=levels(as.factor(comm$Hydropd)))
ordiplot(RDA_veg, display=c('si', 'cn'), type = 'n', main="Vegetation")
points(RDA_veg, display='si', col = c("red", "black", "#56B4E9"), pch = 20, cex=2)
legend('bottomright', col=c("red", "black", "#56B4E9"), pch=20, cex=1.5, legend=c("some", "bare", "vegetated"))
ordiplot(RDA_SO2, display=c('si', 'cn'), type = 'n', main="Sulphate Concentration")
points(RDA_SO2, display='si', col = c("red", "black", "#56B4E9"), pch = 20, cex=2)
legend('bottomright', col=c("red", "black", "#56B4E9"), pch=20, cex=1.5, legend=c("high", "low", "medium"))
par(mfrow = c(1,1))
